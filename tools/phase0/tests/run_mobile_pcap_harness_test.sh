#!/usr/bin/env bash
#
# run_mobile_pcap_harness_test.sh — pcap-driven harness for mobile/security parsers
# (issue #144, 4.2).
#
# Counterpart to run_tcpip_pcap_harness_test.sh (issue #143, 4.1) which targets
# the TCP/IP-stack parsers. This harness targets the mobile/security plugin
# family (S1AP, NGAP, GTPv2, NAS5G, Diameter) plus the TCP/IP parsers they
# depend on (IP, UDP, SCTP, GTP, RADIUS) via src/mmt_mobile and
# src/mmt_tcpip.
#
# What it does:
#   1. Build + install the SDK with BUILD=asan into an isolated prefix.
#   2. Compile tools/phase0/tests/mobile_pcap_harness.c with
...
#   3. Generate synthetic mobile/security pcaps (Diameter, S1AP, NGAP over
#      SCTP; GTP, GTPv2, RADIUS over UDP; SCTP control) via
#      tools/phase0/gen_mobile_pcap.py — pure stdlib, no scapy.
...
#
# Usage: tools/phase0/tests/run_mobile_pcap_harness_test.sh
set -euo pipefail
...
gcc -g -O1 -fsanitize=address,undefined -fno-sanitize-recover=all \
    -o "${BIN}" "${TEST_DIR}/mobile_pcap_harness.c" \
    -I"${PREFIX}/dpi/include" \
    -L"${PREFIX}/dpi/lib" -lmmt_core -ldl -lpcap -lpthread -lm

echo "[3/4] generating synthetic mobile/security pcaps"
mkdir -p "${SYNTH_DIR}"
python3 "${PHASE0_DIR}/gen_mobile_pcap.py" --out-dir "${SYNTH_DIR}"
...
# Synthetic pcaps (cover every mobile/security parser the harness targets).
for p in "${SYNTH_DIR}"/*.pcap; do
    [ -f "$p" ] || continue
    run_pcap "$p" "synthetic $(basename "$p" .pcap)"
done

# Vendored CI pcaps (real captures: still exercised for cross-stack stability;
# they primarily hit TCP/IP, but the harness must not regress on them either).
...
echo ""
echo "Mobile pcap harness: ${checks} pcap(s), $((checks - failures)) passed, ${failures} failed"
if [ "${failures}" -ne 0 ]; then
    echo "✗ Mobile pcap harness: FAIL"
    exit 1
fi
echo "✓ Mobile pcap harness: PASS"
