...
void * ip_sessionizer(void * protocol_context, ipacket_t * ipacket, unsigned index, int * is_new_session)
{
    ...
    if (session) {
        ...
        // if(session->last_packet_direction != packet_direction){
        //     debug("[IP] Sesssion direction changes: %d",packet_direction);
        //     if(session->proto_path.proto_path[index] != PROTO_IP){
        //         debug("[IP] Sesssion path is changed: %d",session->proto_path.proto_path[index]);
