...
int update_path( uint8_t*data_out,  int proto_offset,int type){
		int   rv;
	int header_length;
	int payload_length;
	// Get http2 protocol offset
	...
	if( proto_id != PROTO_HTTP2 )
  	...
  	switch(att_id){
		...
		case(HTTP2_PATH_FUZZ):
			...
			difference_size=update_path((uint8_t*)data_out,proto_offset,HTTP2_PATH_FUZZ);
		...
		case(HTTP2_HEADER_AMPLIFICATION):
			difference_size=update_path((uint8_t*)data_out,proto_offset,HTTP2_HEADER_AMPLIFICATION);
			//printf("[update_http2_data]data_size %d\n",data_size);
