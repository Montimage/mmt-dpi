...
#define PROTO_CLASSIFICATION_DONE                                                                                         \
	0                                    /**< defines that processing is done with the classification process.            \
...
#define PROTO_CLASSIFICATION_UPDATE \
	2                            /**< defines that an already classified protocol was encountered. This is an update */
...
struct mmt_session_struct {
	...
	uint32_t session_timeout_milestone; /**< The time expressed as seconds since Epoch (1st Jan 1970) when the session
	...
	uint8_t is_fragmenting; /** 1 - session is processing a fragmented packet, 0 - session is not processing any
							   fragmented packet*/
	/* Total statistics */
	uint64_t packet_count; /**< tracks the number of packets */
	uint64_t
		packet_cap_count; /**< number of packets which are captured as in this session - include fragmented packets*/
	...
	uint64_t packet_cap_count_direction[2];  /**< Session's packet count ( - include fragmented packets) in both
												directions: initiator <-> remote */
	uint64_t data_cap_volume_direction[2];   /**< Session's data volume ( - include fragmented packets) in both
												directions: initiator <-> remote */
	uint64_t data_packet_count_direction[2]; /**< Session's effective payload packet count in both directions: initiator
												<-> remote */
	uint64_t data_byte_volume_direction[2];  /**< Session's effective payload data volume in both directions: initiator
												<-> remote */
	...
	uint64_t sub_packet_cap_count;  /**< subsession: number of packets which are captured as in this session - include
									   fragmented packets*/
	...
	uint64_t sub_packet_count_direction[2]; /**< subsession: Session's packet count in both directions: initiator <->
											   remote */
	uint64_t
		sub_data_volume_direction[2]; /**< subsession: Session's data volume in both directions: initiator <-> remote */
	uint64_t sub_packet_cap_count_direction[2]; /**< subsession: Session's packet count ( - include fragmented packets)
												   in both directions: initiator <-> remote */
	uint64_t sub_data_cap_volume_direction[2]; /**< subsession: Session's data volume ( - include fragmented packets) in
												  both directions: initiator <-> remote */
	uint64_t sub_data_packet_count_direction[2]; /**< subsession: Session's effective payload packet count in both
													directions: initiator <-> remote */
	uint64_t sub_data_byte_volume_direction[2];  /**< subsession: Session's effective payload data volume in both
													directions: initiator <-> remote */
	// End of children's statistics

	struct timeval s_init_time;          /**< indicates the time when the session was first detected. */
	struct timeval s_last_activity_time; /**< indicates the time when the last activity on this session was detected
											(time of the last packet). */
	struct timeval rtt;                  /**< TCP RTT calculated at connection setup */
	struct timeval s_last_data_packet_time[2]; /**< indicates the time when the last data packet (packet has payload) on
												  this session was detected in both direction: initiator <-> remote. */
	...
	struct mmt_session_struct
		*next; /**< pointer to the next session in the expiry list --- for internal use must not be changed */
	struct mmt_session_struct
		*previous; /**< pointer to the previous session in the expiry list --- for internal use must not be changed */
	...
	uint8_t type : 2;
	uint8_t setup_packet_direction : 1; /**< the direction of the first packet of this session (Lower_toHigher or
										   Higher_to_Lower) */
	uint8_t
		last_packet_direction : 1; /**< the direction of the current packet Lower_toHigher or Higher_to_Lower.
...
//     uint8_t last_packet_direction : 1, setup_packet_direction : 1, type : 2, status :
//     4,packet_direction:1,init_finished:1;
#elif BYTE_ORDER == BIG_ENDIAN
 ...
 */
// This structure is a superset of "attribute_t" defined in public include file "data_defs.h" be careful when modifying
// this.
...
struct attribute_handler_element_struct {
	attribute_internal_t
		*attribute; /**< pointer to the internal attribute structure with which the handler is registered */
...
struct proto_statistics_internal_struct {
	...
	uint64_t timedout_sessions_count;  /**< Total number of timedout sessions (this is the difference between sessions
										  count and ative sessions count) */
...
struct protocol_stack_struct {
	...
	generic_stack_classification_function
		stack_classify;                   /**< The base classification function for the protocol stack. */
...
struct protocol_struct {
	...
	generic_get_attribute_id_by_name
		get_attribute_id_by_name; /**< funtion pointer that returns the protocol's attribute id by name */
	generic_get_attribute_name_by_id
		get_attribute_name_by_id; /**< function pointer that returns the protocol's attribute name by id */
	generic_get_attribute_data_type_by_id
		get_attribute_data_type_by_id; /**< function pointer that returns the data type of an attribute */
	generic_get_attribute_data_length_by_id
		get_attribute_data_length_by_id; /**< function pointer that returns the data length of an attribute */
	generic_get_attribute_position_by_id
		get_attribute_position; /**< function pointer that returns the attribute position in the message */
	generic_is_valid_attribute
		is_valid_attribute; /**< function pointer that indicates if an attribute is valid for this protocol or not */
	generic_get_attribute_scope get_attribute_scope; /**< function pointer that indicates the scope of an attribute */
	generic_get_attribute_extraction_function
		get_attribute_extraction_function; /**< function pointer that indicates the extraction function to use for a
											  given attribute of this protocol */
	generic_comparison_fct session_key_compare; /**< Pointer to the session keys comparison function. Will exist if the
												   proto has a session context. */
...
struct mmt_handler_struct {
	...
	session_timer_handler_t session_timer_handler;  // This is the function registered by user and will be call from
													// function process_timer_handler()
	...
	uint32_t
		fragment_in_packet;  // Number of new fragment in one packet which will trigger the fragmentation evasion: 1
	uint32_t fragmented_packet_in_session;  // Number of fragmented packet in one session which will trigger the
											// fragmentation evasion: 2
 ...
 */
proto_statistics_internal_t *get_child_protocol_stats(proto_statistics_internal_t *proto_stats,
...
/**
 * Iterates through the registered protocol stacks and calls the given callback function for every registered protocol
 * stack.
...
#define mmt_match_strprefix(payload, payload_len, str) \
	mmt_match_prefix((payload), (payload_len), (str), (sizeof(str) - 1))
