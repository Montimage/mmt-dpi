...
void set_metric_grades_values_matrix(application_quality_estimation_internal_t *app_internal_struct)
{
	...
	while (metric != NULL) {
		...
		while (metric_grade != NULL) {
			metric_grades_values_matrix[metric->metric_index][metric_grade->grade_index] = get_function_by_type(
				metric_grade->membership_function_type)(*metric_values_array[metric->metric_index], metric_grade);
...
void get_index_value(application_quality_estimation_internal_t *app_internal_struct, metric_t *quality_metric,
					 double *aggregation_index)
{
	double index_value;
	...
	while (rule != NULL) {
		...
		index_value =
			app_internal_struct->metrics_membership_function_values_matrix[rule_element->metric->metric_index]
																		  [rule_element->metric_grade->grade_index];
		...
		get_aggregation_function(quality_metric->quality_estimation_rules->aggregation_type)(
			rule_index_array, aggregation_index, SAMPLES_NB);
 ...
 */
void calculate_index_array(metric_grade_rule_element_t *output_element, double index_value, int sample_nb,
						   double *rule_index_array)
{
	int step_nb;
	double x, value, step;
	...
	for (step_nb = 0; step_nb <= sample_nb; step_nb++) {
		...
		value = (get_function_by_type(quality_metric_grade->membership_function_type)(x, quality_metric_grade)) *
				index_value;

		if (value <= index_value) {
			rule_index_array[step_nb] = value;
		} else
			(rule_index_array[step_nb] = index_value);
	}
}
...
double estimate_quality_index(application_quality_estimation_internal_t *app_internal_struct)
{
	...
	quality_metric_step = (app_internal_struct->application_quality_estimation->estimation_metrics->metric_range_high -
						   app_internal_struct->application_quality_estimation->estimation_metrics->metric_range_low) /
						  SAMPLES_NB;
	...
	get_index_value(app_internal_struct, app_internal_struct->application_quality_estimation->estimation_metrics,
					aggregation_index_array);
	for (s = 0; s < SAMPLES_NB; s++) {
		input_value = app_internal_struct->application_quality_estimation->estimation_metrics->metric_range_low +
					  s * quality_metric_step;
