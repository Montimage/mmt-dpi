...
{
	if (app_q_est != NULL) {
		application_quality_estimation_internal_t *app_q_est_internal =
			(application_quality_estimation_internal_t *)mmt_malloc(sizeof(application_quality_estimation_internal_t));
		if (app_q_est_internal != NULL) {
			...
			app_q_est_internal->metric_values = mmt_malloc(nb_metrics * sizeof(double *));
			app_q_est_internal->quality_metrics_estimated_values = mmt_malloc(nb_estimation_metrics * sizeof(double));
			if (app_q_est_internal->metric_values == NULL ||
				app_q_est_internal->quality_metrics_estimated_values == NULL) {
				exit(-1);
			}

			// Allocating memory for Membership function Array
			app_q_est_internal->metrics_membership_function_values_matrix = mmt_malloc(nb_metrics * sizeof(double *));
			...
			while (temp_metric != NULL) {
				app_q_est_internal->metrics_membership_function_values_matrix[count] =
					mmt_malloc(temp_metric->nb_grades * sizeof(double));
...
application_quality_estimation_t *init_new_application_quality_estimation_struct(int app_id)
{
	...
	new_application = (application_quality_estimation_t *)mmt_malloc(sizeof(application_quality_estimation_t));
...
application_quality_estimation_rules_t *init_new_app_quality_estimation_rules(int rules_aggregation_type)
{
	...
	new_app_rules = mmt_malloc(sizeof(application_quality_estimation_rules_t));
...
{
	rule_t *new_rule = mmt_malloc(sizeof(rule_t));
	if (new_rule == NULL) {
											  ...
											  int metric_id, int grade_value)
{
	...
	if (!application_struct || !rule) {
		...
		metric_grade_rule_element_t *new_rule_element = mmt_malloc(sizeof(metric_grade_rule_element_t));
		...
		if (QUALITY_ESTIMATION_MODE == MULTI_QUALITY_METRICS) {
			...
			if (application->estimation_metrics == NULL) {
				...
				application->nb_estimation_metrics =
					1;  // It was zero before! anyway we set it to 1 to be on the safe side :)
			} else {
...
{
	metric_t *new_metric = (metric_t *)mmt_malloc(sizeof(metric_t));
	if (new_metric == NULL) {
 ...
 * @return
 */
metric_grade_membership_function_t *init_new_grade_membership_function(int membership_function_type, int grade_value,
																	   int nb_parameters)
{
	metric_grade_membership_function_t *grade_membership_function = (metric_grade_membership_function_t *)mmt_malloc(
		sizeof(metric_grade_membership_function_t) + nb_parameters * sizeof(double));
	...
	memset(grade_membership_function, '\0',
		   sizeof(sizeof(metric_grade_membership_function_t) + nb_parameters * sizeof(double)));
	...
	grade_membership_function->membership_function_parameters =
		(double *)&((char *)grade_membership_function)[sizeof(metric_grade_membership_function_t)];
 ...
 * __
 *   \__
 * param1 is the value at the upper level of the slope and param2 is the distance between the lower and upper levels of
 * the slope
 * @param grade_value
 * @param param1
 * @param param2
...
{
	metric_grade_membership_function_t *grade_membership_function =
		init_new_grade_membership_function(MMT_TRAPEZ_LEFT, grade_value, 2);
 ...
 *    __
 * __/
 * param1 is the value at the lower level of the slope and param2 is the distance between the upper and lower levels of
 * the slope
 * @param grade_value
 * @param param1
 * @param param2
...
{
	metric_grade_membership_function_t *grade_membership_function =
		init_new_grade_membership_function(MMT_TRAPEZ_RIGHT, grade_value, 2);
...
{
	metric_grade_membership_function_t *grade_membership_function =
		init_new_grade_membership_function(MMT_TRAPEZ_CENTER, grade_value, 4);
...
generic_membership_function_by_kpi_and_grade get_function_by_type(int mfb_function_type)
{
	switch (mfb_function_type) {
		...
		case 2:
			return trapezoid_medium;
		case 3:
			return trapezoid_right;
		case 4:
			return guassian_left;
		case 5:
			return guassian_medium;
		case 6:
			return guassian_right;
...
double trapezoid_left(double kpi_value, metric_grade_membership_function_t *mfp_parameters)
{
	...
	if (kpi_value <= mfp_parameters->membership_function_parameters[0]) {
		membershipvalue = 1;
	} else if ((kpi_value >= mfp_parameters->membership_function_parameters[0]) &&
			   kpi_value <= (mfp_parameters->membership_function_parameters[0] +
							 mfp_parameters->membership_function_parameters[1])) {
		membershipvalue = 1 - ((kpi_value - mfp_parameters->membership_function_parameters[0]) /
							   mfp_parameters->membership_function_parameters[1]);
	} else
		membershipvalue = 0;
...
double trapezoid_medium(double kpi_value, metric_grade_membership_function_t *mfp_parameters)
{
	...
	if (kpi_value >=
			(mfp_parameters->membership_function_parameters[0] - mfp_parameters->membership_function_parameters[2]) &&
		kpi_value <= mfp_parameters->membership_function_parameters[0]) {
		membershipvalue = 1 - ((mfp_parameters->membership_function_parameters[0] - kpi_value) /
							   mfp_parameters->membership_function_parameters[2]);
	} else if (kpi_value >= mfp_parameters->membership_function_parameters[0] &&
			   kpi_value <= mfp_parameters->membership_function_parameters[1])
		membershipvalue = 1;
	else if (kpi_value >= mfp_parameters->membership_function_parameters[1] &&
			 kpi_value <= (mfp_parameters->membership_function_parameters[1] +
						   mfp_parameters->membership_function_parameters[3])) {
		membershipvalue = 1 - ((kpi_value - mfp_parameters->membership_function_parameters[1]) /
							   mfp_parameters->membership_function_parameters[3]);
	} else
		membershipvalue = 0;
...
double guassian_left(double kpi_value, metric_grade_membership_function_t *mfp_parameters)
{
	...
	double membershipvalue;
	membershipvalue = (1 / sqrt(2 * M_PI * pow(mfp_parameters->membership_function_parameters[1], 2))) *
					  exp(-(pow(kpi_value - mfp_parameters->membership_function_parameters[0], 2) /
							(2 * pow(mfp_parameters->membership_function_parameters[1], 2))));
...
generic_rule_function get_rule_function_by_rule_type(int rule_type)
{
	switch (rule_type) {
		case AND_RULE_TYPE:
			return (generic_rule_function)get_min;
		case OR_RULE_TYPE:
