...
#define BINARY_64DATA_TYPE_LEN                                                                                      \
	68 /**< Length in bytes of a 64binary data type. A 64binary data type is composed of an int indicating the data \
		  length and a char table of 64 bytes. */
#endif
...
#define BINARY_1024DATA_TYPE_LEN                                                                                     \
	1028 /**< Length in bytes of a 1024binary data type. A 1024binary data type is composed of an int indicating the \
			data length and a char table of 1024 bytes. */
#endif
...
#define STRING_DATA_TYPE_LEN                                                                                      \
	1508 /**< Length in bytes of a string data type. A string data type is composed of an int indicating the data \
			length and a char table of 1504 bytes. */
#endif
...
typedef struct mmt_u16_array_struct {
	uint32_t len;
	uint16_t data[BINARY_64DATA_LEN];
} mmt_u16_array_t;

#ifndef U16_ARRAY_TYPE_LEN
#define U16_ARRAY_TYPE_LEN                                                                                             \
	(sizeof(mmt_u16_array_t)) /**< Length in bytes of a 1024binary data type. A 1024binary data type is composed of an \
								 int indicating the data length and a char table of 1024 bytes. */
#endif
...
#define U32_ARRAY_TYPE_LEN                                                                                             \
	(sizeof(mmt_u32_array_t)) /**< Length in bytes of a 1024binary data type. A 1024binary data type is composed of an \
								 int indicating the data length and a char table of 1024 bytes. */
#endif
...
typedef struct mmt_u64_array_struct {
	uint32_t len;
	uint64_t data[BINARY_64DATA_LEN];
} mmt_u64_array_t;

#ifndef U64_ARRAY_TYPE_LEN
#define U64_ARRAY_TYPE_LEN                                                                                             \
	(sizeof(mmt_u64_array_t)) /**< Length in bytes of a 1024binary data type. A 1024binary data type is composed of an \
								 int indicating the data length and a char table of 1024 bytes. */
#endif
...
enum data_types {
	...
	MMT_BINARY_VAR_DATA,   /**< binary constant value with variable size given by function
							  getExtractionDataSizeByProtocolAndFieldIds */
	MMT_STRING_DATA, /**< text string data constant value. Len plus data. Data is expected to be '\0' terminated and
						maximum BINARY_64DATA_LEN long */
	MMT_STRING_LONG_DATA, /**< text string data constant value. Len plus data. Data is expected to be '\0' terminated
							 and maximum STRING_DATA_LEN long */
	MMT_HEADER_LINE, /**< string pointer value with a variable size. The string is not necessary null terminating */
	MMT_GENERIC_HEADER_LINE, /**< structure representing an RFC2822 header line with null terminating field and value
								elements. */
	MMT_STRING_DATA_POINTER, /**< pointer constant value (size is void *). The data pointed to is of type string with
								null terminating character included */
	MMT_U16_ARRAY,           /** struct mmt_u16_array_t representing an array of uint16_t numbers */
	MMT_U32_ARRAY,           /** struct mmt_u32_array_t representing an array of uint32_t numbers */
	MMT_U64_ARRAY            /** struct mmt_u64_array_t representing an array of uint64_t numbers */
};

// This should be updated whenever data_types is updated.
#define MMT_HIGHER_VALUED_VALID_DATA_TYPE MMT_U64_ARRAY + 1 /**< Defines the higher valued valid data type.*/
