 ...
 */
typedef int (*generic_attribute_extraction_function)(const ipacket_t *packet, unsigned proto_index,
													 attribute_t *extracted_data);

/**
 * Signature of the function for returning the attribute extraction function given the attribute id
 */
typedef generic_attribute_extraction_function (*generic_get_attribute_extraction_function)(uint32_t proto_id,
 ...
 * This is the function that the MMT-extraction core will call to load the plugin. The function should return a positive
 * value on success, 0 otherwise.
 */
...
/**
 * Signature of the function for classifying the protocol corresponding to the encapsulated data. Any implementing
 * function MUST analyze the @param packet and @param header parameters in order to identify the encapsulated protocol.
 * If such a classification is successful, the function MUST return the identifier of the classified protocol. If the
 * classification does not make it possible to identify the encapsulated protocol, the implementing function MUST return
 * 0.
 */
 ...
 */
typedef void *(*generic_sessionizer_function)(void *protocol_context, ipacket_t *ipacket, unsigned previous_index,
											  int *is_new);
 ...
 */
typedef int (*generic_session_context_cleanup_function)(void *protocol_context, mmt_session_t *session_context,
														void *args);
...
MMTAPI int MMTCALL set_classified_proto(ipacket_t *ipacket, unsigned index, classified_proto_t classified_proto);

/**
 * Registers a classification function for the protocol identified by the given @param proto_id. This function should
 * be used when a protocol wants to complement the classification of another protocol (HTTP adding a classification to
 * TCP). For protocols registering their own classification function, this function SHOULD BE AVOIDED, rather use
 * "register_classification_function".
...
MMTAPI int MMTCALL register_classification_function_with_parent_protocol(
	uint32_t proto_id, generic_classification_function classification_fct, int weight);
 ...
 * @return a positive value on success, zero on failure.
 */
MMTAPI int MMTCALL register_classification_function(protocol_t *protocol_struct,
 ...
 * @param classification_fct The classification function.
 * @param weight The weight of this classification function. A classification function with a lower weight
 * will be executed before a classification function with a higher weight if the protocol has multiple registered
 * classification functions. Must be between 10 and 80.
 ...
 */
MMTAPI int MMTCALL register_classification_function_full(protocol_t *protocol_struct,
 ...
 * @return a positive value on success, zero value on failure.
 */
MMTAPI int MMTCALL register_pre_post_classification_functions(protocol_t *protocol_struct,
															  generic_classification_function pre_classification,
															  generic_classification_function post_classification);

/**
 * Registers a sessionizer function along with the session context cleanup function for the protocol identified by the
 * given @param protocol_struct.
 * @param protocol_struct The protocol structure.
 * @param sessionizer_fct The sessionizer function.
 * @param session_context_cleanup_fct The function responsible for cleaning session context. This is required for
 * example when a session times out.
 * @param session_keys_comparison_fct The function for comparing two session keys.
 */
MMTAPI void MMTCALL register_sessionizer_function(protocol_t *protocol_struct,
...
MMTAPI void MMTCALL register_proto_context_init_cleanup_function(
	protocol_t *protocol_struct, generic_proto_context_init_function context_init_fct,
...
MMTAPI void MMTCALL register_session_data_initialization_function(
	protocol_t *protocol_struct, generic_session_data_initialization_function session_data_init_fct);
...
MMTAPI void MMTCALL register_session_data_cleanup_function(
	protocol_t *protocol_struct, generic_session_data_cleanup_function session_data_cleanup_fct);

/**
 * Registers a session data analysis function for the protocol identified by the given @param proto_id. This function
 * should be used in order to complement the analysis of a protocol. For protocols registering their own analysis
 * function, this function SHOULD BE AVOIDED, rather use "register_session_data_analysis_function".
 * @param proto_id The identifier of the protocol that will use the classification function.
...
MMTAPI int MMTCALL register_session_data_analysis_function(
	protocol_t *protocol_struct, generic_session_data_analysis_function session_data_analysis_fct);
...
MMTAPI int MMTCALL register_session_data_analysis_function_full(
	protocol_t *protocol_struct, generic_session_data_analysis_function session_data_analysis_fct, int weight,
	generic_session_data_analysis_function pre_analysis, generic_session_data_analysis_function post_analysis);
 ...
 * @return a positive value on success, zero value on failure.
 */
MMTAPI int MMTCALL register_pre_post_analysis_functions(protocol_t *protocol_struct,
														generic_session_data_analysis_function pre_analysis,
														generic_session_data_analysis_function post_analysis);

/**
 * Registers the protocol's stack given by the stack identifier, name and base classification function. The registration
 * fails if a protocol stack is already registered with the given identifier.
 ...
 * @return a positive value on success, zero value on failure.
 */
MMTAPI int MMTCALL register_protocol_stack(uint32_t s_id, char *s_name, generic_stack_classification_function fct);

/**
 * Registers the protocol's stack given by the stack identifier, name, base classification function, internal data and
 * cleanup function. The registration fails if a protocol stack is already registered with the given identifier.
 * @param s_id The unique identifier of the protocol stack to register.
 * @param s_name The name of the protocol stack to register.
 * @param fct The base classification function corresponding to the protocol stack to register.
 ...
 * @return a positive value on success, zero value on failure.
 */
MMTAPI int MMTCALL register_protocol_stack_full(uint32_t s_id, char *s_name, generic_stack_classification_function fct,
 ...
 * @return a positive value on success, zero value on failure.
 * <p> A positive value is returned if the given identifier does not correspond to any registered protocol stack. This
 * is not considered as failure.
 */
MMTAPI int MMTCALL unregister_protocol_stack(uint32_t s_id);
 ...
 */
MMTAPI int MMTCALL is_free_protocol_id_for_registractionl(uint32_t proto_id);
 ...
 */
MMTAPI protocol_t *MMTCALL get_protocol_struct_for_registration_if_free(uint32_t proto_id);
 ...
 */
MMTAPI protocol_t *MMTCALL init_protocol_struct_for_registration(uint32_t proto_id, const char *protocol_name);
 ...
 * @return a positive value on success (valid attribute and not already registered), a zero value is returned on
 * failure.
 */
MMTAPI int MMTCALL register_attribute_with_protocol(protocol_t *protocol_struct,
 ...
 */
MMTAPI int MMTCALL register_protocol(protocol_t *protocol_struct, uint32_t proto_id);
 ...
 * @return PROTO_REGISTERED on success, PROTO_NOT_REGISTERED on failure.
 */
MMTAPI int MMTCALL unregister_protocol_by_id(uint32_t proto_id);
 ...
 * @return PROTO_REGISTERED on success, PROTO_NOT_REGISTERED on failure.
 */
MMTAPI int MMTCALL unregister_protocol_by_name(char *proto_name);
 ...
 */
MMTAPI protocol_t *MMTCALL get_protocol_struct_by_id(uint32_t proto_id);

//  - - - - - - - - - - - - - - - - - -
//  P R O T O C O L   A C C E S S O R S
//  - - - - - - - - - - - - - - - - - -

MMTAPI int MMTCALL get_proto_attribute_position(protocol_t *proto, uint32_t proto_id, uint32_t attr_id);

MMTAPI int MMTCALL get_proto_attribute_length(protocol_t *proto, uint32_t proto_id, uint32_t attr_id);

MMTAPI int MMTCALL get_proto_attribute_id(protocol_t *proto, uint32_t proto_id, const char *attr_name);

MMTAPI const char *MMTCALL get_proto_attribute_name(protocol_t *proto, uint32_t proto_id, uint32_t attr_id);

MMTAPI int MMTCALL get_proto_attribute_type(protocol_t *proto, uint32_t proto_id, uint32_t attr_id);

MMTAPI int MMTCALL get_proto_attribute_scope(protocol_t *proto, uint32_t proto_id, uint32_t attr_id);

MMTAPI int MMTCALL is_valid_proto_attribute(protocol_t *proto, uint32_t proto_id, uint32_t attr_id);
