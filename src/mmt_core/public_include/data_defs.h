...
#define Max_Alias_Len                    \
	64 /**< Max length of an alias name. \
 ...
 * if the value of key1 given by its address is strictly lower than the value of key2; false is returned otherwise.
 * (strict weak ordering operation).
 */
 ...
 * if the value of key1 is strictly lower than the value of key2; false is returned otherwise. (strict weak ordering
 * operation).
 */
...
typedef struct pkthdr {
	...
	unsigned int original_caplen; /**< Original capture len of the packet when it was captured by interface - not count
									 with reassembly data*/
	unsigned int original_len;    /**< Original capture len of the packet when it was captured by interface - not count
									 with reassembly data*/
...
struct ipacket_struct {
	...
	uint8_t is_completed[PROTO_PATH_SIZE]; /**< 1 - yes, 0 - no: Indicate if the packet is completed to go to parse to
											  next protocol*/
	...
	proto_hierarchy_t
		internal_proto_hierarchy; /**< internal: - never modify it. the protocol layers corresponding to this packet */
	proto_hierarchy_t internal_proto_headers_offset; /**< internal: - never modify it.  the offsets corresponding to the
														protocol layers of this packet */
	proto_hierarchy_t internal_proto_classif_status; /**< internal: - never modify it.  the classification status of the
														protocols in the path */
	...
	mmt_tcpip_internal_packet_t
		*internal_packet;       /**< pointer to opaque packet structure. for internal use only. Must never be changed */
	...
	const u_char *original_data; /**< internal: - never modify it. pointer to the original packet data. It will be
									different than ipacket->data in case of IP assembled data*/
};

/**
 * Defines the identification info of a protocol attribute. This identification is defined by the tuple: protocol id and
 * attribute id.
 * @obsolete this structure should never be used! It is maintained for backward compatibility. It will be removed from
 * future versions.
 */
 ...
 */
// This structure is a subset of "attribute_internal_t" defined in private include file "packet_processing.h" be careful
// when modifying this.
...
struct proto_statistics_struct {
	...
	uint64_t packets_count_direction[2]; /**< Total number of UL/DL packets seen by the protocol  on a particular
											protocol path */
	uint64_t
		data_volume_direction[2]; /**< Total UL/DL data volume seen by the protocol  on a particular protocol path */
	uint64_t payload_volume_direction[2]; /**< Total UL/DL payload data volume seen by the protocol  on a particular
											 protocol path */
	uint64_t sessions_count; /**< Total number of sessions seen by the protocol  on a particular protocol path */
	uint64_t timedout_sessions_count; /**< Total number of timedout sessions (this is the difference between sessions
										 count and active sessions count) on a particular protocol path */
 ...
 */
MMTAPI const char *MMTCALL get_protocol_name_by_id(uint32_t proto_id);
 ...
 */
MMTAPI const char *MMTCALL get_attribute_name_by_protocol_and_attribute_ids(uint32_t proto_id, uint32_t attribute_id);
 ...
 */
MMTAPI int MMTCALL get_packet_offset_at_index(const ipacket_t *ipacket, unsigned index);
 ...
 */
MMTAPI uint32_t MMTCALL get_protocol_id_at_index(const ipacket_t *ipacket, unsigned index);
 ...
 */
MMTAPI unsigned MMTCALL get_protocol_index_by_id(const ipacket_t *ipacket, uint32_t proto_id);
 ...
 * If the protocol name is not valid or the protocol does not appear in the protocol hierarchy, -1 is returned.
 */
MMTAPI unsigned MMTCALL get_protocol_index_by_name(const ipacket_t *ipacket, const char *proto_name);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_id_from_packet(const ipacket_t *ipacket);
 ...
 */
MMTAPI void *MMTCALL get_user_session_context_from_packet(const ipacket_t *ipacket);
 ...
 */
MMTAPI void MMTCALL set_user_session_context_for_packet(const ipacket_t *ipacket, void *user_data);

/**
 * Returns a pointer to initialized the session data of the protocol at the given index. NULL if the protocol has no
 * registered session data.
 * @param ipacket the packet structure
 * @param index the index of the protocol in the path
 * @return pointer to initialized the session data of the protocol at the given index. NULL if the protocol has no
 * registered session data.
 */
MMTAPI void *MMTCALL get_proto_session_data_from_packet(const ipacket_t *ipacket, unsigned index);
 ...
 */
MMTAPI mmt_session_t *MMTCALL get_session_from_packet(const ipacket_t *ipacket);
 ...
 */
MMTAPI mmt_session_t *MMTCALL get_session_parent(const mmt_session_t *session);
 ...
 */
MMTAPI mmt_handler_t *MMTCALL get_session_handler(const mmt_session_t *session);
 ...
 */
MMTAPI uint32_t MMTCALL get_session_protocol_index(const mmt_session_t *session);
 ...
 */
MMTAPI const proto_hierarchy_t *MMTCALL get_session_protocol_hierarchy(const mmt_session_t *session);
 ...
 * @return the associated session identifier.
 */
MMTAPI uint64_t MMTCALL get_session_id(const mmt_session_t *session);
 ...
 * @return the associated session context.
 */
MMTAPI void *MMTCALL get_user_session_context(const mmt_session_t *session);
 ...
 * registered session data.
 */
MMTAPI void *MMTCALL get_proto_session_data(const mmt_session_t *session, unsigned index);
 ...
 * @param index the index of the protocol in the path
 */
MMTAPI void MMTCALL set_proto_session_data(mmt_session_t *session, void *proto_data, unsigned index);
 ...
 * @param user_data a pointer to a user-defined session context
 */
MMTAPI void MMTCALL set_user_session_context(mmt_session_t *session, void *user_data);

/**
 * Returns the number of packets seen by this session (not include subsession)
 * @param session the session structure
 * @return the number of packets seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_packet_count(const mmt_session_t *session);

/**
 * Returns the number of packets captured by this session (not include subsession)
 * @param session the session structure
 * @return the number of packets captured by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_packet_cap_count(const mmt_session_t *session);


/**
 * Returns the volume of data captured by this session (not include subsession)
 * @param session the session structure
 * @return the volume of data captured by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_data_cap_volume(const mmt_session_t *session);

/**
 * Returns the uplink number of packets seen by this session (not include subsession)
 * @param session the session structure
 * @return the uplink number of packets seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_ul_packet_count(const mmt_session_t *session);

/**
 * Returns the uplink number of packets captured by this session (not include subsession)
 * @param session the session structure
 * @return the uplink number of packets captured by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_ul_cap_packet_count(const mmt_session_t *session);

/**
 * Returns the downlink number of packets seen by this session (not include subsession)
 * @param session the session structure
 * @return the downlink number of packets seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_dl_packet_count(const mmt_session_t *session);

/**
 * Returns the downlink number of packets captured by this session (not include subsession)
 * @param session the session structure
 * @return the downlink number of packets captured by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_dl_cap_packet_count(const mmt_session_t *session);

/**
 * Returns total volume in bytes seen by this session (not include subsession)
 * @param session the session structure
 * @return total volume in bytes seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_byte_count(const mmt_session_t *session);

/**
 * Returns total uplink volume in bytes seen by this session (not include subsession)
 * @param session the session structure
 * @return total uplink volume in bytes seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_ul_byte_count(const mmt_session_t *session);

/**
 * Returns total downlink volume in bytes seen by this session (not include subsession)
 * @param session the session structure
 * @return total downlink volume in bytes seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_dl_byte_count(const mmt_session_t *session);

/**
 * Returns total uplink volume in bytes captured by this session (not include subsession)
 * @param session the session structure
 * @return total uplink volume in bytes captured by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_ul_cap_byte_count(const mmt_session_t *session);

/**
 * Returns total downlink volume in bytes captured by this session (not include subsession)
 * @param session the session structure
 * @return total downlink volume in bytes captured by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_dl_cap_byte_count(const mmt_session_t *session);

/**
 * Returns the number of data packets seen by this session (not include subsession). <br>
 ...
 * @param session the session structure
 * @return the number of data packets seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_data_packet_count(const mmt_session_t *session);

/**
 * Returns the uplink number of data packets seen by this session (not include subsession). <br>
 * A data packet is expected to contain effective application data.
 * TCP ACKs are not considered as data packets and will not be counted therefore.
 * @param session the session structure
 * @return the uplink number of data packets seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_ul_data_packet_count(const mmt_session_t *session);

/**
 * Returns the number of downlink data packets seen by this session (not include subsession). <br>
 * A data packet is expected to contain effective application data.
 * TCP ACKs are not considered as data packets and will not be counted therefore.
 * @param session the session structure
 * @return the downlink number of data packets seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_dl_data_packet_count(const mmt_session_t *session);

/**
 * Returns the total number of data bytes seen by this session (not include subsession). <br>
 * Only effective application payload data will be accounted.
 * TCP ACKs are not considered as data packets and will not be counted therefore.
 * @param session the session structure
 * @return total data volume in bytes seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_data_byte_count(const mmt_session_t *session);

/**
 * Returns the total uplink number of data bytes seen by this session (not include subsession). <br>
 * Only effective application payload data will be accounted.
 * TCP ACKs are not considered as data packets and will not be counted therefore.
 * @param session the session structure
 * @return total uplink data volume in bytes seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_ul_data_byte_count(const mmt_session_t *session);

/**
 * Returns the total downlink number of data bytes seen by this session (not include subsession). <br>
 * Only effective application payload data will be accounted.
 * TCP ACKs are not considered as data packets and will not be counted therefore.
 * @param session the session structure
 * @return total downlink data volume in bytes seen by this session (not include subsession).
 */
MMTAPI uint64_t MMTCALL get_session_dl_data_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_packet_cap_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_data_cap_volume(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_ul_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_ul_cap_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_dl_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_dl_cap_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_ul_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_dl_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_ul_cap_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_dl_cap_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_data_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_ul_data_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_dl_data_packet_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_data_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_ul_data_byte_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint64_t MMTCALL get_session_total_dl_data_byte_count(const mmt_session_t *session);

// END OF GET TOTAL SESSION STATISTICS
 ...
 */
MMTAPI struct timeval MMTCALL get_session_init_time(const mmt_session_t *session);
 ...
 */
MMTAPI struct timeval MMTCALL get_session_last_activity_time(const mmt_session_t *session);
 ...
 * @param tv pointer to timeval struct where the session last activity time will be copied
 */
MMTAPI struct timeval MMTCALL get_session_last_data_packet_time_by_direction(const mmt_session_t *session,
																			 uint8_t direction);
 ...
 */
MMTAPI struct timeval MMTCALL get_session_rtt(const mmt_session_t *session);
 ...
 */
MMTAPI uint16_t MMTCALL get_session_content_class_id(const mmt_session_t *session);
 ...
 */
MMTAPI uint16_t MMTCALL get_session_content_type_id(const mmt_session_t *session);
 ...
 */
MMTAPI uint32_t MMTCALL get_session_content_flags(const mmt_session_t *session);
 ...
 */
MMTAPI uint32_t MMTCALL get_session_retransmission_count(const mmt_session_t *session);
 ...
 */
MMTAPI uint32_t MMTCALL get_session_outoforder_count(const mmt_session_t *session);
 ...
 */
MMTAPI const mmt_session_t MMTCALL *get_session_next(const mmt_session_t *session);
 ...
 */
MMTAPI const mmt_session_t MMTCALL *get_session_previous(const mmt_session_t *session);
 ...
 */
MMTAPI const proto_hierarchy_t MMTCALL *get_session_proto_path_direction(const mmt_session_t *session, int direction);
 ...
 * @return           protocol path
 */
MMTAPI uint8_t MMTCALL get_session_setup_direction(const mmt_session_t *session);
 ...
 * @return           protocol path
 */
MMTAPI uint8_t MMTCALL get_session_last_packet_direction(const mmt_session_t *session);
 ...
 */
MMTAPI unsigned int MMTCALL htoi(char *binary_data, const char *hex_data, int len);
 ...
 */
MMTAPI int MMTCALL get_data_size_by_proto_and_field_ids(uint32_t proto_id, uint32_t attribute_id);
 ...
 */
MMTAPI uint32_t MMTCALL get_data_size_by_data_type(uint32_t data_type);
 ...
 * @param proto_id the identifier of the protocol
 * @param attribute_id the identifier of the attribute
 * @return the position of the attribute in the message. POSITION_NOT_KNOWN is returned if the proto_id and attribute_id
 * couple are not valid.
 */
MMTAPI int MMTCALL get_field_position_by_protocol_and_field_ids(uint32_t proto_id, uint32_t attribute_id);
 ...
 */
MMTAPI int MMTCALL is_protocol_attribute(uint32_t proto_id, uint32_t attribute_id);
 ...
 */
MMTAPI uint32_t MMTCALL get_protocol_id_by_name(const char *protocol_name);
 ...
 */
MMTAPI uint32_t MMTCALL get_attribute_id_by_protocol_and_attribute_names(const char *protocol_name,
 ...
 * @return the identifier (positive value) of the attribute on success, 0 otherwise.
 */
MMTAPI uint32_t MMTCALL get_attribute_id_by_protocol_id_and_attribute_name(uint32_t proto_id,
																		   const char *attribute_name);
 ...
 */
MMTAPI long MMTCALL get_attribute_data_type(uint32_t proto_id, uint32_t attribute_id);
 ...
 */
MMTAPI int MMTCALL get_attribute_scope(uint32_t proto_id, uint32_t attribute_id);
 ...
 */
MMTAPI uint32_t MMTCALL short_time_diff(struct timeval *starttime, struct timeval *finishtime);
 ...
 */
MMTAPI uint32_t MMTCALL get_classification_threshold(void);
 ...
 */
MMTAPI int MMTCALL proto_hierarchy_to_str(const proto_hierarchy_t *proto_hierarchy, char *dest);
 ...
 */
MMTAPI const char *MMTCALL get_application_name(const proto_hierarchy_t *proto_hierarchy);
 ...
 */
MMTAPI char MMTCALL mmt_toupper(char in);
 ...
 */
MMTAPI char MMTCALL mmt_tolower(char in);
 ...
 * @return Zero if both strings are case insensitivly equal, positive value if to indicate that the first character that
 * does not match has a greater case insensitive value in \first than in \second; a negative value indicates the
 * opposite.
 */
MMTAPI int MMTCALL mmt_strcasecmp(const char *first, const char *second);
 ...
 *         a positive value indicates that the first character that does not match has a greater case insensitive value
 * in \first than in \second; a negative value indicates the opposite.
 */
MMTAPI int MMTCALL mmt_strncasecmp(const char *first, const char *second, size_t max);
 ...
 */
MMTAPI int MMTCALL mmt_strcmp(const char *first, const char *second);
 ...
 * @param second string to be compared.
 * @param max maximum number of characters to compare.
 * @return Zero indicates that the characters compared in both strings form the same string; positive value if to
 * indicate that the first character that does not match has a greater value in \first than in \second; a negative value
 * indicates the opposite.
 */
MMTAPI int MMTCALL mmt_strncmp(const char *first, const char *second, size_t max);
 ...
 * If the resulting string would be longer than \len - 1 characters, the remaining characters are discarded and not
 * stored. In this case, the value returned by the function would be higher than \len. <br> A terminating null character
