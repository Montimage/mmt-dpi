...
int utime_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
...
int uargs_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	extracted_data->data = packet->p_hdr->user_args;
	return 1;
}

int probe_id_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	*((unsigned int *)extracted_data->data) = packet->p_hdr->probe_id;
	return 1;
}

int source_id_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	*((unsigned int *)extracted_data->data) = packet->p_hdr->source_id;
	return 1;
}

int packet_direction_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
...
int proto_hierarchy_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	// int proto_offset = get_packet_offset_at_index(packet, proto_index);
	// protocol_t *protocol_struct = get_protocol_struct_by_id(protocol_id);
	// int attribute_offset = protocol_struct->get_attribute_position(protocol_id, attribute_id);
...
int plen_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	// int proto_offset = get_packet_offset_at_index(packet, proto_index);
	// protocol_t *protocol_struct = get_protocol_struct_by_id(protocol_id);
	// int attribute_offset = protocol_struct->get_attribute_position(protocol_id, attribute_id);
...
int session_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	// int proto_offset = get_packet_offset_at_index(packet, proto_index);
	// protocol_t *protocol_struct = get_protocol_struct_by_id(protocol_id);
	// int attribute_offset = protocol_struct->get_attribute_position(protocol_id, attribute_id);
...
int classified_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	if (packet->session == NULL) {
		return 0;
...
int packet_id_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
...
static attribute_metadata_t meta_attributes_metadata[META_ATTRIBUTES_NB] = {
	{META_PACKET_DIRECTION, META_PACKET_DIRECTION_ALIAS, MMT_U32_DATA, sizeof(int), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 packet_direction_extraction},
	{META_UARGS, META_UARGS_ALIAS, MMT_DATA_POINTER, sizeof(void *), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 uargs_extraction},
	{META_UTIME, META_UTIME_ALIAS, MMT_DATA_TIMEVAL, sizeof(struct timeval), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 utime_extraction},
	{META_P_LEN, META_P_LEN_ALIAS, MMT_U32_DATA, sizeof(int), POSITION_NOT_KNOWN, SCOPE_PACKET, plen_extraction},
	{META_PROTO_H, META_PROTO_H_ALIAS, MMT_DATA_PATH, sizeof(proto_hierarchy_t), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_hierarchy_extraction},
	{META_SESSION, META_SESSION_ALIAS, MMT_DATA_POINTER, sizeof(void *), POSITION_NOT_KNOWN, SCOPE_SESSION,
	 session_extraction},
	{META_CLASSIFIED, META_CLASSIFIED_ALIAS, MMT_U8_DATA, sizeof(char), POSITION_NOT_KNOWN, SCOPE_SESSION,
	 classified_extraction},
	{META_PROBE_ID, META_PROBE_ID_ALIAS, MMT_U32_DATA, sizeof(int), POSITION_NOT_KNOWN, SCOPE_SESSION,
	 probe_id_extraction},
	{META_SOURCE_ID, META_SOURCE_ID_ALIAS, MMT_U32_DATA, sizeof(int), POSITION_NOT_KNOWN, SCOPE_SESSION,
	 source_id_extraction},
	{META_PACKET_INDEX, META_PACKET_INDEX_ALIAS, MMT_U64_DATA, sizeof(uint64_t), POSITION_NOT_KNOWN, SCOPE_SESSION,
	 packet_id_extraction},
};
