...
static mmt_log_config_t g_log_config = {.global_level = MMT_LOG_INFO,
...
static const char *g_log_level_names[] = {
	[MMT_LOG_NONE] = "NONE", [MMT_LOG_ERROR] = "ERROR", [MMT_LOG_WARN] = "WARN",
	[MMT_LOG_INFO] = "INFO", [MMT_LOG_DEBUG] = "DEBUG", [MMT_LOG_TRACE] = "TRACE"};

/* Log category names */
static const char *g_log_category_names[] = {[MMT_LOG_CAT_GENERAL] = "GENERAL",
...
static void format_timestamp(char *buffer, size_t size)
{
	...
	snprintf(buffer, size, "%04d-%02d-%02d %02d:%02d:%02d.%03ld", tm_info.tm_year + 1900, tm_info.tm_mon + 1,
			 tm_info.tm_mday, tm_info.tm_hour, tm_info.tm_min, tm_info.tm_sec, ts.tv_nsec / 1000000);
}
...
void mmt_log_init_with_config(const mmt_log_config_t *config)
{
	...
	if (g_log_config.output_mode == MMT_LOG_OUTPUT_FILE && g_log_config.log_file_path != NULL) {
...
bool mmt_log_is_enabled(mmt_log_level_t level, mmt_log_category_t category)
{
	...
	if (!g_log_config.category_enabled[category])
		return false;
			 ...
			 const char *format, ...)
{
	...
	if (g_log_config.output_mode == MMT_LOG_OUTPUT_CALLBACK && g_log_config.callback != NULL) {
