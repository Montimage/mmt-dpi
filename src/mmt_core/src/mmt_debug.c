    ...
    FILE *output)
{
    ...
    while (pos < length) {
        size_t line_len = (length - pos > BYTES_PER_LINE) ?
                         BYTES_PER_LINE : (length - pos);
    ...
    FILE *output)
{
    ...
    fprintf(out, "\n%s [offset %zu-%zu]:\n", label, start, end);
    mmt_dump_packet(data + start, end - start, start,
                   MMT_DUMP_HEX | MMT_DUMP_ASCII | MMT_DUMP_OFFSETS,
                   out);
}
    ...
    mmt_error_stat_entry_t *entry)
{
    ...
    if (entry != NULL) {
        memcpy(entry, &g_error_entries[error_code],
               sizeof(mmt_error_stat_entry_t));
    }
    ...
    size_t max_entries)
{
    ...
    for (size_t n = 0; n < max_entries; n++) {
        ...
        if (max_idx >= 0) {
            memcpy(&entries[count], &g_error_entries[max_idx],
                   sizeof(mmt_error_stat_entry_t));
            count++;
        } else {
            break;
...
void mmt_error_stats_print(FILE *output, size_t top_n)
{
    ...
    for (size_t i = 0; i < count; i++) {
        fprintf(out, "%2zu. [%3d] %s\n",
                i + 1, entries[i].error_code,
                mmt_error_to_string(entries[i].error_code));
...
void mmt_profile_end(mmt_profile_point_t *point)
{
    ...
    point->elapsed_ns = (end_time.tv_sec - point->start_time.tv_sec) * 1000000000ULL +
                       (end_time.tv_nsec - point->start_time.tv_nsec);

    MMT_LOG_PERF(MMT_LOG_CAT_PERFORMANCE,
                 "%s: %lu ns", point->name, point->elapsed_ns);
