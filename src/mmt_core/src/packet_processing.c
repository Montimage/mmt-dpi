 ...
 */
void update_proto_stats_on_session_timeout(mmt_session_t *timed_out_session,
										   proto_statistics_internal_t *parent_proto_stats);
...
static protocol_stack_t dummy_stack = {0, "dummy", dummy_stack_classification, NULL, NULL};

static protocol_t *configured_protocols[PROTO_MAX_IDENTIFIER];
static pthread_rwlock_t protocol_registry_lock =
	PTHREAD_RWLOCK_INITIALIZER; /* Phase 3: Thread safety for protocol registry */
...
int get_attribute_id_by_name_from_protocol_map(uint32_t proto_id, const char *attribute_name)
{
	if (_is_registered_protocol(proto_id)) {
		attribute_metadata_t *attr = (attribute_metadata_t *)find_key_value(
			configured_protocols[proto_id]->attributes_names_map, (void *)attribute_name);
		if (attr != NULL) {
			return attr->id;
		}
	}
...
{
	if (_is_registered_protocol(proto_id)) {
		attribute_metadata_t *attr = (attribute_metadata_t *)find_int_key_value(
			configured_protocols[proto_id]->attributes_map, (uint32_t)attribute_id);
		if (attr != NULL) {
			return attr->alias;
		}
	}
...
generic_attribute_extraction_function get_attribute_extraction_fct_by_id_from_protocol_map(uint32_t proto_id,
...
int validate_attribute_metadata(attribute_metadata_t *attribute_meta_data)
{
	...
	if (attribute_meta_data->id == 0)
		return false;
	// The name must have a length of at least one and less than Max_Alias_Len - PVS V560: due to the decleration: char
	// alias[Max_Alias_Len + 1]; -> but it could be a fault possitive because some time we have terminated character in
	// the alias
	if ((strlen(attribute_meta_data->alias) == 0) || (strlen(attribute_meta_data->alias) > Max_Alias_Len))
		return false;
	...
	if (!((attribute_meta_data->scope & SCOPE_ON_DEMAND) || (attribute_meta_data->scope & SCOPE_EVENT)))
		return false;
...
int register_attribute_with_protocol(protocol_t *proto, attribute_metadata_t *attribute_meta_data)
{
	...
	if (validate_attribute_metadata(attribute_meta_data) == true) {
		attribute_metadata_t *attr =
			(attribute_metadata_t *)find_int_key_value(proto->attributes_map, (uint32_t)attribute_meta_data->id);
		attribute_metadata_t *attr_by_name =
			(attribute_metadata_t *)find_key_value(proto->attributes_names_map, attribute_meta_data->alias);
		if (attr == NULL && attr_by_name == NULL) {
			...
			if (attr != NULL) {
				...
				if (!insert_key_value(proto->attributes_names_map, (void *)attr->alias, (void *)attr)) {
					fprintf(stderr,
							"[error] register_attribute_with_protocol - Failed to execute insert_key_value()\n");
				};
				return 1;
			}
		}
...
{
	((generic_protocol_attribute_iteration_callback)((struct internal_attribute_iterator_struct *)args)->iterator_fct)(
		(attribute_metadata_t *)value, ((struct internal_attribute_iterator_struct *)args)->proto_id,
		((struct internal_attribute_iterator_struct *)args)->args);
}

void internal_handler_iterator_callback(void *key, void *value, void *args)
{
	((generic_handler_iteration_callback)((struct internal_handler_iterator_struct *)args)->iterator_fct)(
		value, ((struct internal_attribute_iterator_struct *)args)->args);
}

void iterate_through_protocol_attributes(uint32_t proto_id, generic_protocol_attribute_iteration_callback iterator_fct,
										 void *args)
{
	if (_is_registered_protocol(proto_id) == PROTO_REGISTERED) {
		...
		int_mapspace_iteration_callback(proto->attributes_map, internal_attribute_iterator_callback,
										(void *)&temp_attribute_iterator_struct);
	}
}
...
{
	return str_len <= payload_len ? mmt_memcmp(payload, str, str_len) == 0 : 0;
}
...
void iterate_through_mmt_handlers(generic_handler_iteration_callback iterator_fct, void *args)
{
	...
	mapspace_iteration_callback(mmt_configured_handlers_map, internal_handler_iterator_callback,
								(void *)&temp_handler_iterator_struct);
}

int register_session_timeout_handler(mmt_handler_t *mmt_h,
									 generic_session_timeout_handler_function session_expiry_handler_fct, void *args)
{
...
int register_session_timer_handler(mmt_handler_t *mmt_h,
								   generic_session_timer_handler_function session_timer_handler_fct, void *args,
...
static inline void cleanup_timedout_sessions(mmt_session_t *timed_out_session)
{
	...
	for (; i < timed_out_session->proto_path.len; i++) {
		if (_is_registered_protocol(timed_out_session->proto_path.proto_path[i])) {
			if (configured_protocols[timed_out_session->proto_path.proto_path[i]]->session_data_cleanup != NULL) {
				((generic_session_data_cleanup_function)
					 configured_protocols[timed_out_session->proto_path.proto_path[i]]
						 ->session_data_cleanup)(timed_out_session, i);
			}
		}
	}
	...
	((generic_session_context_cleanup_function)((protocol_instance_t *)timed_out_session->protocol_container_context)
		 ->protocol->session_context_cleanup)((protocol_instance_t *)timed_out_session->protocol_container_context,
...
void force_sessions_timeout(void *timeout_milestone, void *milestone_sessions_list, void *args)
{
	...
	while (timed_out_session != NULL) {
		...
		if (mmt_handler->session_expiry_handler.handler_fct) {
			mmt_handler->session_expiry_handler.handler_fct(safe_to_delete_session,
															mmt_handler->session_expiry_handler.args);
		}
	...
	if (mmt_handler->session_timer_handler.session_timer_handler_fct != NULL) {
		while (current_session != NULL) {
			...
			if (mmt_handler->session_timer_handler.no_fragmented && current_session->is_fragmenting) {
				...
				mmt_handler->session_timer_handler.session_timer_handler_fct(current_session,
																			 mmt_handler->session_timer_handler.args);
				current_session = current_session->next;
			}
		}
	} else {
...
/**
 * Register classificaiton function internal - insert this function into right position: for example: classify FTP after
 * HTTP -> HTTP 10, FTP 50
 ...
 *                            1 successful
 */
int register_classification_function_internal(protocol_t *proto, generic_classification_function classification_fct,
											  int weight)
{
	if (weight < 0)
		weight = 0;
...
{
	if (classification_fct != NULL) {
		return register_classification_function_internal(proto, classification_fct,
														 50);  // TODO: replace with a definition
	}
	return 0;
}

int register_pre_post_classification_functions(protocol_t *proto, generic_classification_function pre_classification,
...
int register_classification_function_full(protocol_t *proto, generic_classification_function classification_fct,
										  int weight, generic_classification_function pre_classification,
										  generic_classification_function post_classification)
{
	if (weight < 10)
		weight = 10;
...
void register_proto_context_init_cleanup_function(protocol_t *proto,
...
void register_session_data_initialization_function(protocol_t *proto,
												   generic_session_data_initialization_function session_data_init_fct)
{
	proto->session_data_init = (void *)session_data_init_fct;
}

void register_session_data_cleanup_function(protocol_t *proto,
											generic_session_data_cleanup_function session_data_cleanup_fct)
{
	proto->session_data_cleanup = (void *)session_data_cleanup_fct;
}

int register_data_analysis_function_internal(protocol_t *proto, generic_session_data_analysis_function analysis_fct,
											 int weight)
{
	if (weight < 0)
		weight = 0;
...
int register_session_data_analysis_function_with_protocol(
	uint32_t proto_id, generic_session_data_analysis_function session_data_analysis_fct, int weight)
{
...
{
	if (session_data_analysis_fct != NULL) {
		return register_data_analysis_function_internal(proto, session_data_analysis_fct,
														50);  // TODO: replace with a definition
	}
	return 0;
}

int register_pre_post_analysis_functions(protocol_t *proto, generic_session_data_analysis_function pre_analysis,
...
int register_session_data_analysis_function_full(protocol_t *proto,
												 generic_session_data_analysis_function session_data_analysis_fct,
												 int weight, generic_session_data_analysis_function pre_analysis,
												 generic_session_data_analysis_function post_analysis)
{
	if (weight < 10)
		weight = 10;
...
int is_free_protocol_id_for_registractionl(uint32_t proto_id)
{
	if (proto_id >= PROTO_MAX_IDENTIFIER)
		return 0;  // The prtocol id is not valid

	protocol_t *proto = configured_protocols[proto_id];
	if (!proto)
		return 1;  // protocol just doesn't exist (plugin ?)
	if (proto->is_registered)
		return 0;  // The protocol is already registered
	...
	for (; i < PROTO_MAX_IDENTIFIER; i++) {
		if (mmt_handler->configured_protocols[i].protocol != NULL) {
			if (mmt_handler->configured_protocols[i].protocol->is_registered) {
				if (mmt_handler->configured_protocols[i].protocol->has_session &&
				...
				if (mmt_handler->configured_protocols[i].protocol->protocol_context_cleanup != NULL) {
					((generic_proto_context_cleanup_function)mmt_handler->configured_protocols[i]
						 .protocol->protocol_context_cleanup)(
						&mmt_handler->configured_protocols[i],
						mmt_handler->configured_protocols[i].protocol->protocol_context_args);
				}
			}
		}
	}
...
int proto_packet_count_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	protocol_instance_t *configured_protocol =
		&(packet->mmt_handler)->configured_protocols[packet->proto_hierarchy->proto_path[proto_index]];
...
int proto_data_volume_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	protocol_instance_t *configured_protocol =
		&(packet->mmt_handler)->configured_protocols[packet->proto_hierarchy->proto_path[proto_index]];
	proto_statistics_internal_t *proto_stats = configured_protocol->proto_stats;
...
int proto_payload_volume_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	protocol_instance_t *configured_protocol =
		&(packet->mmt_handler)->configured_protocols[packet->proto_hierarchy->proto_path[proto_index]];
	proto_statistics_internal_t *proto_stats = configured_protocol->proto_stats;
...
int proto_first_packet_time_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	protocol_instance_t *configured_protocol =
		&(packet->mmt_handler)->configured_protocols[packet->proto_hierarchy->proto_path[proto_index]];
	proto_statistics_internal_t *proto_stats = configured_protocol->proto_stats;
...
int proto_last_packet_time_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	protocol_instance_t *configured_protocol =
		&(packet->mmt_handler)->configured_protocols[packet->proto_hierarchy->proto_path[proto_index]];
	proto_statistics_internal_t *proto_stats = configured_protocol->proto_stats;
...
int proto_header_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
...
int proto_data_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	int proto_offset = get_packet_offset_at_index(packet, proto_index);
	extracted_data->data = (void *)&packet->data[proto_offset];
	return 1;
...
int proto_data_len_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	int proto_offset = get_packet_offset_at_index(packet, proto_index);
...
int proto_payload_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	int proto_offset;
	if (proto_index + 1 == packet->proto_hierarchy->len) {
...
int proto_session_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
...
int proto_session_id_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
	if (packet->session == NULL) {
...
int proto_stats_extraction(const ipacket_t *packet, unsigned proto_index, attribute_t *extracted_data)
{
...
static attribute_metadata_t proto_stats_attributes_metadata[PROTO_STATS_ATTRIBUTES_NB] = {
	{PROTO_HEADER, PROTO_HEADER_LABEL, MMT_DATA_POINTER, sizeof(void *), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_header_extraction},
	{PROTO_DATA, PROTO_DATA_LABEL, MMT_DATA_POINTER, sizeof(void *), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_data_extraction},
	{PROTO_PAYLOAD, PROTO_PAYLOAD_LABEL, MMT_DATA_POINTER, sizeof(void *), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_payload_extraction},
	{PROTO_PACKET_COUNT, PROTO_PACKET_COUNT_LABEL, MMT_U64_DATA, sizeof(uint64_t), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_packet_count_extraction},
	{PROTO_DATA_VOLUME, PROTO_DATA_VOLUME_LABEL, MMT_U64_DATA, sizeof(uint64_t), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_data_volume_extraction},
	{PROTO_PAYLOAD_VOLUME, PROTO_PAYLOAD_VOLUME_LABEL, MMT_U64_DATA, sizeof(uint64_t), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_payload_volume_extraction},
	{PROTO_STATISTICS, PROTO_STATISTICS_LABEL, MMT_STATS, sizeof(void *), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_stats_extraction},
	{PROTO_FIRST_PACKET_TIME, PROTO_FIRST_PACKET_TIME_LABEL, MMT_DATA_TIMEVAL, sizeof(struct timeval),
	 POSITION_NOT_KNOWN, SCOPE_PACKET, proto_first_packet_time_extraction},
	{PROTO_LAST_PACKET_TIME, PROTO_LAST_PACKET_TIME_LABEL, MMT_DATA_TIMEVAL, sizeof(struct timeval), POSITION_NOT_KNOWN,
	 SCOPE_PACKET, proto_last_packet_time_extraction},
	{PROTO_DATA_LEN, PROTO_DATA_LEN_LABEL, MMT_U64_DATA, sizeof(uint64_t), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_data_len_extraction},

};

static attribute_metadata_t proto_session_attr_metadata[PROTO_SESSION_ATTRIBUTES_NB] = {
	{PROTO_SESSION, PROTO_SESSION_LABEL, MMT_DATA_POINTER, sizeof(void *), POSITION_NOT_KNOWN, SCOPE_EVENT,
	 proto_session_extraction},
	{PROTO_SESSION_ID, PROTO_SESSION_ID_LABEL, MMT_U64_DATA, sizeof(uint64_t), POSITION_NOT_KNOWN, SCOPE_PACKET,
	 proto_session_id_extraction},
};
...
mmt_handler_t *mmt_init_handler(uint32_t stacktype, uint32_t options, char *errbuf)
{
	...
	for (i = 0; i < PROTO_MAX_IDENTIFIER; i++) {
		...
		if (new_handler->configured_protocols[i].protocol->has_session == HAS_SESSION_CONTEXT) {
			new_handler->configured_protocols[i].sessions_map =
				init_map_space(new_handler->configured_protocols[i].protocol->session_key_compare);
		}
		...
		if (new_handler->configured_protocols[i].protocol->protocol_context_init != NULL) {
			new_handler->configured_protocols[i].args =
				(void *)((generic_proto_context_init_function)new_handler->configured_protocols[i]
							 .protocol->protocol_context_init)(
					(void *)&new_handler->configured_protocols[i],
					new_handler->configured_protocols[i].protocol->protocol_context_args);
		}
...
struct attribute_internal_struct *get_registered_attribute_internal_struct(const ipacket_t *ipacket, uint32_t proto_id,
																		   uint32_t attribute_id, unsigned index)
{
	...
	tmp_attr_ref =
		mmt_handler->proto_registered_attributes[proto_id];  // This is safe as we are sure the protocol is registered
...
#ifdef DEBUG
	(void)fprintf(stderr,
				  "[debug] internal_extract_attribute (%p) : packet - %" PRIu64 ", proto_id - %" PRIu32
				  ", attribute_id - %" PRIu32 " - index: %u\n",
				  tmp_attr_ref->extraction_function, ipacket->packet_id, tmp_attr_ref->proto_id, tmp_attr_ref->field_id,
				  index);
#endif /*DEBUG*/
 ...
 */
static inline void *_get_attribute_extracted_data_at_index(const ipacket_t *ipacket, uint32_t proto_id,
														   uint32_t attribute_id, unsigned index)
{
#ifdef DEBUG
	(void)fprintf(stderr,
				  "[debug] _get_attribute_extracted_data_at_index : packet - %" PRIu64 ", proto_id - %" PRIu32
				  ", attribute_id - %" PRIu32 " - index: %u\n",
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[error] get_attribute_extracted_data_at_index : packet - %" PRIu64 ", proto_id - %" PRIu32
					  ", attribute_id - %" PRIu32 " - index: %u : invalid index (%u)\n",
	...
	if (proto_id != ipacket->proto_hierarchy->proto_path[index]) {
		...
		(void)fprintf(stderr,
					  "[error] get_attribute_extracted_data_at_index : packet - %" PRIu64 ", proto_id - %" PRIu32
					  ", attribute_id - %" PRIu32 " - index: %u : unexpected protocol_id (%u)\n",
	...
	if (!_is_registered_protocol(proto_id)) {
		...
		(void)fprintf(stderr,
					  "[error] get_attribute_extracted_data_at_index : packet - %" PRIu64 ", proto_id - %" PRIu32
					  ", attribute_id - %" PRIu32 " - index: %u : unregistered protocol_id (%u)\n",
					  ipacket->packet_id, proto_id, attribute_id, index, proto_id);
#endif /*DEBUG*/
		return NULL;
	}
	...
	struct attribute_internal_struct *tmp_attr_ref =
		get_registered_attribute_internal_struct(ipacket, proto_id, attribute_id, index);
	if (tmp_attr_ref == NULL) {
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[error] get_attribute_extracted_data_at_index : packet - %" PRIu64 ", proto_id - %" PRIu32
					  ", attribute_id - %" PRIu32 " - index: %u : can't retrieve attribute internal structure\n",
					  ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
		return NULL;
	}
	...
	if (tmp_attr_ref->scope & SCOPE_ON_DEMAND) {
		if (internal_extract_attribute(ipacket, tmp_attr_ref, index)) {
			...
			// #ifdef DEBUG
			//             (void)fprintf( stderr,"[error] get_attribute_extracted_data_at_index : packet - %"PRIu64",
			//             proto_id - %"PRIu32", attribute_id - %"PRIu32" - index: %u : attribute data is null
			//             (1/2)\n",ipacket->packet_id, proto_id, attribute_id, index );
			// #endif /*DEBUG*/
	...
	if (!(tmp_attr_ref->scope & SCOPE_EVENT)) {
		(void)fprintf(stderr,
					  "[error] get_attribute_extracted_data_at_index : packet - %" PRIu64 ", proto_id - %" PRIu32
					  ", attribute_id - %" PRIu32 " - index: %u : unexpected failure\n",
					  ipacket->packet_id, proto_id, attribute_id, index);
	}
#endif /*DEBUG*/
	return NULL;
...
void *get_attribute_extracted_data_at_index(const ipacket_t *ipacket, uint32_t proto_id, uint32_t attribute_id,
											unsigned index)
{
#ifdef DEBUG
	(void)fprintf(stderr,
				  "[debug] get_attribute_extracted_data_at_index: calling _get_attribute_extracted_data_at_index: "
				  "packet - %" PRIu64 ", proto_id - %" PRIu32 " , attribute_id - %" PRIu32 ", index - %u \n",
				  ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
	return _get_attribute_extracted_data_at_index(ipacket, proto_id, attribute_id, index);
}

attribute_t *get_extracted_attribute_at_index(const ipacket_t *ipacket, uint32_t proto_id, uint32_t attribute_id,
											  unsigned index)
{
	if ((int)index < 0 || index >= ipacket->proto_hierarchy->len) {
		// the given index is not valid
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[error] get_extracted_attribute_at_index: invalid index : packet - %" PRIu64
					  ", proto_id - %" PRIu32 " , attribute_id - %" PRIu32 ", index - %u \n",
					  ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
		return NULL;
	}
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[error] get_extracted_attribute_at_index: unexpected protocol_id : packet - %" PRIu64
					  ", proto_id - %" PRIu32 " , attribute_id - %" PRIu32 ", index - %u \n",
					  ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
		return NULL;
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[error] get_extracted_attribute_at_index: unregistered protocol_id : packet - %" PRIu64
					  ", proto_id - %" PRIu32 " , attribute_id - %" PRIu32 ", index - %u \n",
					  ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
		return NULL;
	...
	if (tmp_attr_ref == NULL) {
		...
		(void)fprintf(
			stderr,
			"[error] get_extracted_attribute_at_index: can't retrieve attribute internal structure: packet - %" PRIu64
			", proto_id - %" PRIu32 " , attribute_id - %" PRIu32 ", index - %u \n",
			ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
		return NULL;
	...
	if (tmp_attr_ref->scope & SCOPE_ON_DEMAND) {
		if (internal_extract_attribute(ipacket, tmp_attr_ref, index)) {
			// return the attribute's data
			// #ifdef DEBUG
			//             (void)fprintf( stderr,"get_extracted_attribute_at_index: attribute data is not null (1/2):
			//             packet - %"PRIu64", proto_id - %"PRIu32" , attribute_id - %"PRIu32", index - %u \n",
			//             ipacket->packet_id,proto_id,attribute_id, index);
			// #endif /*DEBUG*/
			tmp_attr_ref->status = ATTRIBUTE_CONSUMED;
...
#ifdef DEBUG
			(void)fprintf(stderr,
						  "[error] get_extracted_attribute_at_index: attribute data is null (2/2): packet - %" PRIu64
						  ", proto_id - %" PRIu32 " , attribute_id - %" PRIu32 ", index - %u \n",
						  ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
			return (attribute_t *)tmp_attr_ref;
...
#ifdef DEBUG
	(void)fprintf(stderr,
				  "[error] get_extracted_attribute_at_index : packet - %" PRIu64 ", proto_id - %" PRIu32
				  ", attribute_id - %" PRIu32 " - index: %u : unexpected failure\n",
				  ipacket->packet_id, proto_id, attribute_id, index);
#endif /*DEBUG*/
	return NULL;
 ...
 * @return a pointer to the extracted data if it exists, NULL otherwise.
 */
void *_get_attribute_extracted_data_at_index_by_name(const ipacket_t *ipacket, const char *protocol_name,
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_attribute_extracted_data_at_index_by_name(): invalid index - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s, index - %u \n",
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_attribute_extracted_data_at_index_by_name(): unknown protocol name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s, index - %u \n",
					  ipacket->packet_id, protocol_name, attribute_name, index);
#endif /*DEBUG*/
		return NULL;
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_attribute_extracted_data_at_index_by_name(): unknown attribute name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s, index - %u \n",
					  ipacket->packet_id, protocol_name, attribute_name, index);
#endif /*DEBUG*/
		return NULL;
...
#ifdef DEBUG
	(void)fprintf(stderr,
				  "[debug] _get_attribute_extracted_data_at_index_by_name: calling "
				  "_get_attribute_extracted_data_at_index: packet - %" PRIu64
				  ", protocol_name - %s , attribute_name - %s, index - %u \n",
				  ipacket->packet_id, protocol_name, attribute_name, index);
#endif /*DEBUG*/
	return _get_attribute_extracted_data_at_index(ipacket, proto_id, attribute_id, index);
...
attribute_t *get_extracted_attribute_at_index_by_name(const ipacket_t *ipacket, const char *protocol_name,
													  const char *attribute_name, unsigned index)
{
	if ((int)index < 0 || index >= ipacket->proto_hierarchy->len) {
		// the given index is not valid
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_extracted_attribute_at_index_by_name(): invalid index - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s, index - %u \n",
					  ipacket->packet_id, protocol_name, attribute_name, index);
#endif /*DEBUG*/
		return NULL;
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_extracted_attribute_at_index_by_name(): unknown protocol name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s, index - %u \n",
					  ipacket->packet_id, protocol_name, attribute_name, index);
#endif /*DEBUG*/
		return NULL;
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_extracted_attribute_at_index_by_name(): unknown attribute name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s, index - %u \n",
					  ipacket->packet_id, protocol_name, attribute_name, index);
#endif /*DEBUG*/
		return NULL;
...
void *get_attribute_extracted_data_by_name(const ipacket_t *ipacket, const char *protocol_name,
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_attribute_extracted_data_by_name(): unknown protocol name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s\n",
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "[debug] get_attribute_extracted_data_by_name(): unknown attribute name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s\n",
					  ipacket->packet_id, protocol_name, attribute_name);
#endif /*DEBUG*/
		return NULL;
...
attribute_t *get_extracted_attribute_by_name(const ipacket_t *ipacket, const char *protocol_name,
											 const char *attribute_name)
{
	uint32_t proto_id, attribute_id;
	proto_id = get_protocol_id_by_name(protocol_name);
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "get_extracted_attribute_by_name(): unknown protocol name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s\n",
					  ipacket->packet_id, protocol_name, attribute_name);
#endif /*DEBUG*/
		return NULL;
...
#ifdef DEBUG
		(void)fprintf(stderr,
					  "get_extracted_attribute_by_name(): unknown attribute name - %" PRIu64
					  ", protocol_name - %s , attribute_name - %s\n",
					  ipacket->packet_id, protocol_name, attribute_name);
#endif /*DEBUG*/
		return NULL;
...
// TODO: this function does not take into account protocol encapsulation where more than one occurrence of the same
// protocol exists in the path
...
#ifdef DEBUG
			(void)fprintf(stderr,
						  "[debug] get_attribute_extracted_data: calling _get_attribute_extracted_data_at_index: "
						  "packet - %" PRIu64 ", proto_id - %" PRIu32 " , field_id - %" PRIu32 ", index - %u \n",
...
#ifdef DEBUG
	(void)fprintf(stderr,
				  "[debug] get_attribute_extracted_data(): proto_id not found in pathpacket - %" PRIu64
				  ", proto_id - %" PRIu32 " , field_id - %" PRIu32 ", index - %u \n",
				  ipacket->packet_id, proto_id, field_id, index);
#endif /*DEBUG*/
...
void *get_attribute_extracted_data_encap_index(const ipacket_t *ipacket, uint32_t proto_id, uint32_t field_id,
...
int is_registered_attribute(mmt_handler_t *mmt_handler, uint32_t proto_id, uint32_t field_id)
{
	...
	while (tmp_attribute != NULL) {
		if (proto_id == tmp_attribute->proto_id && field_id == tmp_attribute->field_id)
			return 1;
...
static inline struct attribute_internal_struct *get_registered_attribute(mmt_handler_t *mmt_handler, uint32_t proto_id,
																		 uint32_t field_id)
{
...
int has_registered_attribute_handler(mmt_handler_t *mmt_handler, uint32_t proto_id, uint32_t attribute_id)
{
	...
	while (tmp_attribute != NULL) {
		if (proto_id == tmp_attribute->proto_id && attribute_id == tmp_attribute->field_id &&
...
int is_registered_attribute_handler(mmt_handler_t *mmt_handler, uint32_t proto_id, uint32_t attribute_id,
...
int unregister_extraction_attribute_by_name(mmt_handler_t *mmt_handler, const char *protocol_name,
											const char *attribute_name)
{
	uint32_t proto_id, attribute_id;
	proto_id = get_protocol_id_by_name(protocol_name);
...
int unregister_extraction_attribute(mmt_handler_t *mmt_handler, uint32_t proto_id, uint32_t field_id)
{
	...
	//     2- The attribute registration count is equal to one, We decrement its registration count, two cases are
	//     possible
	...
	if (!temp_attr_proto_list->registration_count &&
		!temp_attr_proto_list->handlers_count) {  // Both values are non positive! we delete it
...
int unregister_attribute_handler(mmt_handler_t *mmt_handler, uint32_t proto_id, uint32_t attribute_id,
								 attribute_handler_function handler_fct)
{
	...
	if ((temp_attr->attribute_handler == NULL) && !(temp_attr->scope & SCOPE_EVENT)) {
		...
		attribute_handler_element_t *temp_attr_handler_elem =
			mmt_handler->proto_registered_attribute_handlers[proto_id];
		attribute_handler_element_t *safe_to_delete_attr_handler_elem = NULL;

		while (temp_attr_handler_elem != NULL) {
			if (temp_attr_handler_elem->attribute->proto_id == proto_id &&
				temp_attr_handler_elem->attribute->field_id == attribute_id) {
				break;
			}
...
int register_extraction_attribute_by_name(mmt_handler_t *mmt_handler, const char *protocol_name,
										  const char *attribute_name)
{
	uint32_t proto_id, attribute_id;
	proto_id = get_protocol_id_by_name(protocol_name);
	...
	if (proto != NULL) {
		...
		if (!extract_attribute) {
			...
			if (extract_attribute == NULL) {
				...
				extract_attribute->position_in_packet =
					get_field_position_by_protocol_and_field_ids(proto_id, field_id);
				...
				struct attribute_internal_struct *registered_attr =
					mmt_handler->proto_registered_attributes[extract_attribute->proto_id];
				...
				if (registered_attr == NULL) {
					...
					if (extract_attribute->field_id < registered_attr->field_id) {
						...
						// The attribute to register should be inserted between registered_attr and
						// registered_attr->next
...
int register_attribute_handler(mmt_handler_t *mmt_handler, uint32_t proto_id, uint32_t attribute_id,
							   attribute_handler_function handler_fct, void *handler_condition, void *user_args)
{
	int retval = 0;

	if (is_registered_attribute_handler(mmt_handler, proto_id, attribute_id, handler_fct)) {
		return 0;  // TODO: error codes should be added to differentiate between registration failed and handler already
				   // exists.
	}
	...
	if (new_attribute_handler == NULL) {
		if (retval)
			unregister_extraction_attribute(mmt_handler, proto_id, attribute_id);  // If we get here then the attribute
	...
	if (attr->attribute_handler == NULL) {
		...
		if (!(attr->scope & SCOPE_EVENT)) {
			...
			attribute_handler_element_t *attr_handler_elem =
				(attribute_handler_element_t *)mmt_malloc(sizeof(attribute_handler_element_t));
			...
			if (mmt_handler->proto_registered_attribute_handlers[proto_id] == NULL) {
				...
				attribute_handler_element_t *registered_attr_handler =
					mmt_handler->proto_registered_attribute_handlers[proto_id];
				if (attr_handler_elem->attribute->field_id < registered_attr_handler->attribute->field_id) {
					...
					while (registered_attr_handler->next != NULL) {
						if (attr_handler_elem->attribute->field_id <
							registered_attr_handler->next->attribute->field_id) {
							break;
						}
...
int register_attribute_handler_by_name(mmt_handler_t *mmt_handler, const char *protocol_name,
									   const char *attribute_name, attribute_handler_function handler_fct,
...
int register_packet_handler(mmt_handler_t *mmt_handler, int packet_handler_id, generic_packet_handler_callback function,
							void *args)
{
...
#ifdef DEBUG
			(void)fprintf(stderr,
						  "[debug] debug_extracted_attributes_printout_handler: calling "
						  "_get_attribute_extracted_data_at_index: packet - %" PRIu64 ", proto_id - %" PRIu32
						  " , field_id - %" PRIu32 ", index - %u \n",
						  ipacket->packet_id, tmp_attribute->proto_id, tmp_attribute->field_id, proto_index);
			(void)fprintf(
				stderr,
				"[debug] debug_extracted_attributes_printout_handler: calling _get_attribute_extracted_data_at_index: "
				"packet - %" PRIu64 ", proto_id - %s , field_id - %s, index - %u \n",
				ipacket->packet_id, get_protocol_name_by_id(tmp_attribute->proto_id),
				get_attribute_name_by_protocol_and_attribute_ids(tmp_attribute->proto_id, tmp_attribute->field_id),
				proto_index);
#endif /*DEBUG*/
			data = _get_attribute_extracted_data_at_index(ipacket, tmp_attribute->proto_id, tmp_attribute->field_id,
														  proto_index);
			if (!quiet && data != NULL) {
...
int proto_session_management(ipacket_t *ipacket, protocol_instance_t *configured_protocol, unsigned index)
{
	...
	if (configured_protocol->protocol->has_session) {  // Sessionize packet only if such a function exists!
		session = (mmt_session_t *)((generic_sessionizer_function)configured_protocol->protocol->sessionize)(
			configured_protocol, ipacket, index, &is_new_session);
		/*
		...
		if (session != NULL) {  // Check if a session has been detected. This is
			if (is_new_session) {
				...
				memcpy(&session->proto_headers_offset, ipacket->proto_headers_offset,
					   sizeof(int) + 4 * ipacket->proto_headers_offset->len);
				memcpy(&session->proto_classif_status, ipacket->proto_classif_status,
					   sizeof(int) + 4 * ipacket->proto_classif_status->len);
				...
				if (ipacket->session == NULL) {
					...
					session->parent_session = NULL;  // TODO: parent should be set here. If the ipacket is alreay
													 // associated to a session, then it is the parent of this one!
					ipacket->session = session;
				} else {
					// Embedded session; set its parent
					session->parent_session = ipacket->session;
					// Share the session data from the parent session up to the index of the new detected encapsulated
					// session
					unsigned i;
					for (i = 0; i < index; i++) {
				...
				if (configured_protocol->protocol->session_data_init != NULL) {
					// debug("[PACKET_PROCESS-]> session_data_init - 0 : %"PRIu64"
					// %p",ipacket->packet_id,(generic_session_data_initialization_function)
					// configured_protocol->protocol->session_data_init);
					((generic_session_data_initialization_function)configured_protocol->protocol->session_data_init)(
						ipacket, index);
				}
				...
				if (ipacket->p_hdr->ts.tv_sec >
					session->s_last_activity_time
						.tv_sec) {  // No need to update the timeout if we are still in the same second
					// if(!session->force_timeout) { //Sessions with force timeout should not be updated! they need to
					// timeout :)
					if (update_session_timeout_milestone(mmt_handler,
														 session->session_timeout_delay + ipacket->p_hdr->ts.tv_sec,
														 session->session_timeout_milestone, session) == 0) {
						process_outofmemory_force_sessions_timeout(ipacket->mmt_handler, ipacket);
						insert_session_timeout_milestone(
							mmt_handler, session->session_timeout_delay + ipacket->p_hdr->ts.tv_sec, session);
					}
					session->session_timeout_milestone = session->session_timeout_delay + ipacket->p_hdr->ts.tv_sec;
					//}
				}
		...
		// If this is the case, initialize its session data if required and copy its registered attributes to the
		// session context
		if ((ipacket->session != NULL) &&
			((classify_status == PROTO_CLASSIFICATION_DETECTION) || (classify_status == PROTO_RECLASSIFICATION) ||
			 (classify_status == PROTO_CLASSIFICATION_UPDATE))) {
			// Initialize its session data if such initialization function exists
			if (configured_protocol->protocol->session_data_init != NULL) {
				// debug("[PACKET_PROCESS-]> session_data_init - 1 : %"PRIu64"
				// %p",ipacket->packet_id,(generic_session_data_initialization_function)
				// configured_protocol->protocol->session_data_init);
				((generic_session_data_initialization_function)configured_protocol->protocol->session_data_init)(
					ipacket, index);
...
int set_classified_proto(ipacket_t *ipacket, unsigned index, classified_proto_t classified_proto)
{
	...
	if (index + 1 > ipacket->proto_hierarchy->len) {
		...
		// Set the protocol in the path and update the offsets! We are in the same layer, a protocol reclassification
		// occurred! e.g. Uknown -> HTTP
		ipacket->proto_hierarchy->proto_path[index] = classified_proto.proto_id;
		ipacket->proto_headers_offset->proto_path[index] = classified_proto.offset;
 ...
 */
static inline proto_statistics_internal_t *_create_protocol_stats_instance(
	protocol_instance_t *proto, proto_statistics_internal_t *parent_proto_stats)
{
	proto_statistics_internal_t *proto_stats =
		(proto_statistics_internal_t *)mmt_malloc(sizeof(proto_statistics_internal_t));
...
proto_statistics_internal_t *create_protocol_stats_instance(protocol_instance_t *proto,
															proto_statistics_internal_t *parent_proto_stats)
{
 ...
 */
static inline proto_statistics_internal_t *_get_child_protocol_stats(proto_statistics_internal_t *proto_stats,
 ...
 */
static inline proto_statistics_internal_t *_get_protocol_stats_from_parent(
	protocol_instance_t *proto, proto_statistics_internal_t *parent_proto_stats)
{
	proto_statistics_internal_t *proto_stats;
...
proto_statistics_internal_t *get_protocol_stats_from_parent(protocol_instance_t *proto,
															proto_statistics_internal_t *parent_proto_stats)
{
...
void print_protocol_stats(FILE *f, protocol_instance_t *proto)
{
	...
	while (proto_stats) {
		fprintf(f,
				"Proto %u: \nnb\tpackets %" PRIu64 " --- byte count %" PRIu64 " --- sessions count %" PRIu64
				" --- timeout sessions count %" PRIu64 "\n",
				proto_stats->proto->protocol->proto_id, proto_stats->packets_count, proto_stats->data_volume,
										   ...
										   proto_statistics_internal_t *parent_proto_stats)
{
	...
	int i = 0;
	for (; i < timed_out_session->proto_path.len; i++) {
		proto_stats = _get_protocol_stats_from_parent(
			&(timed_out_session->mmt_handler)->configured_protocols[timed_out_session->proto_path.proto_path[i]],
			proto_stats);
 ...
 */
static inline proto_statistics_internal_t *update_proto_stats_on_packet(ipacket_t *ipacket,
																		protocol_instance_t *configured_protocol,
																		proto_statistics_internal_t *parent_stats,
																		uint32_t proto_offset, unsigned index)
{
	if (likely(isProtocolStatisticsEnabled(ipacket->mmt_handler))) {
		...
		if (likely(proto_stats)) {
			...
			if (likely(configured_protocol->protocol->proto_id == 178 ||
					   configured_protocol->protocol->proto_id == 179 ||
					   configured_protocol->protocol->proto_id == 182)) {
 ...
 */
static inline proto_statistics_internal_t *update_proto_stats_on_new_session(ipacket_t *ipacket,
																			 protocol_instance_t *configured_protocol,
																			 proto_statistics_internal_t *parent_stats,
																			 int new_session, uint32_t proto_offset,
																			 unsigned index)
{
...
int proto_packet_classify_next(ipacket_t *ipacket, protocol_instance_t *configured_protocol, unsigned index)
{
	...
	if (configured_protocol->protocol->classify_next.status) {
		...
		if (configured_protocol->protocol->classify_next.classify_protos &&
			classif_status) {  // Classify next proto only when such a function exists!
			mmt_classify_me_t *temp = configured_protocol->protocol->classify_next.classify_protos;
			// Checking for the port number ??????
			for (; temp != NULL; temp = temp->next) {
				classif_status = temp->classify_me(
					ipacket, index);  // TODO: check the return value and make the corresponding action accordingly!!!
				// // LN: check if the classify return 1-> do not need to go to check other protocol
				if (classif_status &
					3) {  // Short for classif_status == 1 || classif_status == 2 || classif_status == 3
					// printf("\n-]> Classified for protocol %d: %"PRIu64" - %d - %p -
...
void fire_evasion_event(ipacket_t *ipacket, uint32_t proto_id, unsigned proto_index, unsigned evasion_id, void *data)
{
	...
	if (mmt_handler->evasion_handler) {
		mmt_handler->evasion_handler->function(ipacket, proto_id, proto_index, evasion_id, data,
											   mmt_handler->evasion_handler->args);
	} else {
#ifdef DEBUG
...
void proto_process_attribute_handlers(ipacket_t *ipacket, unsigned index)
{
	...
	attribute_handler_element_t *attribute_handler =
		mmt_handler->proto_registered_attribute_handlers[ipacket->proto_hierarchy->proto_path[index]];
	while (attribute_handler != NULL) {
		...
		if (attribute_handler->attribute->status == ATTRIBUTE_SET) {
			...
			while (attr_handler_fct != NULL) {
				attr_handler_fct->handler_fct(ipacket, (attribute_t *)attribute_handler->attribute,
											  attr_handler_fct->args);
				attr_handler_fct = attr_handler_fct->next;
			}
...
{
	if ((ipacket->data != ipacket->original_data) && (ipacket->mmt_handler->link_layer_stack->stack_id == DLT_EN10MB)) {
...
void clean_packet_with_reassembly(ipacket_t *ipacket)
{
	if (ipacket->session) {  // Only packet which has session need to be clean those information
		if (ipacket->proto_headers_offset)
...
int proto_packet_process(ipacket_t *ipacket, proto_statistics_internal_t *parent_stats, unsigned index)
{
	...
	protocol_instance_t *configured_protocol =
		&(ipacket->mmt_handler)->configured_protocols[ipacket->proto_hierarchy->proto_path[index]];
	...
	if (is_new_session == NEW_SESSION) {
		parent_stats =
			update_proto_stats_on_new_session(ipacket, configured_protocol, (proto_statistics_internal_t *)parent_stats,
											  is_new_session, proto_offset, index);
		fire_attribute_event(ipacket, configured_protocol->protocol->proto_id, PROTO_SESSION, index,
							 (void *)ipacket->session);
			...
			if (ipacket->session->proto_path.len > 0) {
				...
				if (ipacket->session->proto_path_direction[proto_direction].len != proto_path_len) {
					...
					for (i = 0; i < proto_path_len; i++) {
						ipacket->session->proto_path_direction[proto_direction].proto_path[i] =
							ipacket->session->proto_path.proto_path[i];
					}
					// debug("[IP] Update protocol path direction: %d", proto_direction);
				} else {
					if (ipacket->session->proto_path_direction[proto_direction].proto_path[proto_path_len - 1] !=
						ipacket->session->proto_path.proto_path[proto_path_len - 1]) {
						ipacket->session->proto_path_direction[proto_direction].proto_path[proto_path_len - 1] =
							ipacket->session->proto_path.proto_path[proto_path_len - 1];
					}
				}
			}
...
void generic_data_extraction(unsigned protocol_index, ipacket_t *ipacket)
{
	...
	tmp_attr_ref = mmt_handler->proto_registered_attributes[proto_id];  // This is safe as we are sure the protocol is
																		// registered (check done in "packet_extract")
...
{
	if (len < 2)
		return -1;  // not less than 1 character (".")
...
int mmt_timeval_sprintf(char *buff, int len, attribute_internal_t *attr)
{
	...
	// return value will be higher than the given length and the user would be able to detect
	// the truncation.
	return snprintf(buff, len, "%lu.%06lu", ((struct timeval *)attr->data)->tv_sec,
					((struct timeval *)attr->data)->tv_usec);
}
...
int mmt_attr_sprintf(char *buff, int len, attribute_t *a)
{
	...
	switch (attr->data_type) {
		...
		case MMT_U16_ARRAY:
			return mmt_u16_array_sprintf(buff, len, attr);
		case MMT_U32_ARRAY:
...
{
	return fprintf(f, "Attribute %s.%s = %c\n", get_protocol_name_by_id(attr->proto_id),
				   get_attribute_name_by_protocol_and_attribute_ids(attr->proto_id, attr->field_id),
				   *(char *)attr->data);
}
...
int mmt_mac_format(FILE *f, attribute_internal_t *attr)
{
	...
	if (mmt_mac_sprintf(buff, MMT_MAC_STRLEN, attr)) {
		return fprintf(f, "Attribute %s.%s = %s\n", get_protocol_name_by_id(attr->proto_id),
					   get_attribute_name_by_protocol_and_attribute_ids(attr->proto_id, attr->field_id), buff);
	}
	return -1;
}
...
int mmt_timeval_format(FILE *f, attribute_internal_t *attr)
{
	return fprintf(f, "Attribute %s.%s  = %lu.%06lu\n", get_protocol_name_by_id(attr->proto_id),
				   get_attribute_name_by_protocol_and_attribute_ids(attr->proto_id, attr->field_id),
				   ((struct timeval *)attr->data)->tv_sec, ((struct timeval *)attr->data)->tv_usec);
}
...
int mmt_string_format(FILE *f, attribute_internal_t *attr)
{
	mmt_binary_var_data_t *b = (mmt_binary_var_data_t *)attr->data;
	return fprintf(f, "Attribute %s.%s = %s\n", get_protocol_name_by_id(attr->proto_id),
				   get_attribute_name_by_protocol_and_attribute_ids(attr->proto_id, attr->field_id), (char *)&b->data);
}
int mmt_string_pointer_format(FILE *f, attribute_internal_t *attr)
{
	int ret =
		fprintf(f, "Attribute %s.%s = %s\n", get_protocol_name_by_id(attr->proto_id),
				get_attribute_name_by_protocol_and_attribute_ids(attr->proto_id, attr->field_id), (char *)attr->data);
	// free((char*)attr->data);
	return ret;
}
...
{
	return fprintf(f, "Attribute %s.%s = %s\n", get_protocol_name_by_id(attr->proto_id),
				   get_attribute_name_by_protocol_and_attribute_ids(attr->proto_id, attr->field_id), "TODO");
}
