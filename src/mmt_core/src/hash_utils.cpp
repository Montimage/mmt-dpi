...
int delete_session_from_protocol_context(void *protocol_context, void *key)
{
	...
	/* Phase 3: Write lock for session deletion */
	pthread_rwlock_wrlock(&proto_inst->session_lock);
	result = delete_key_value(proto_inst->sessions_map, key);
	pthread_rwlock_unlock(&proto_inst->session_lock);
...
void clear_sessions_from_protocol_context(void *protocol_context)
{
	...
	/* Phase 3: Write lock for clearing all sessions */
	pthread_rwlock_wrlock(&proto_inst->session_lock);
	clear_map_space(proto_inst->sessions_map);
	pthread_rwlock_unlock(&proto_inst->session_lock);
}
...
void protocol_sessions_iteration_callback(void *protocol_context, generic_mapspace_iteration_callback fct, void *args)
{
	...
	if (proto_inst->sessions_map != NULL) {
		/* Phase 3: Read lock for session iteration */
		pthread_rwlock_rdlock(&proto_inst->session_lock);
		mapspace_iteration_callback(proto_inst->sessions_map, fct, args);
		pthread_rwlock_unlock(&proto_inst->session_lock);
	}
}
//////////////// Wrapper End
									 ...
									 mmt_session_t *session)
{
	...
	if (it != m->end()) {
		...
		// old_timeout); session already existed in the old
