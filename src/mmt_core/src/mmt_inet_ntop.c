...
static const char *mmt_inet_ntop4(const struct in_addr *addr, char *buf, socklen_t len)
{
...
static const char *mmt_inet_ntop6(const struct in6_addr *addr, char *dst, socklen_t size)
{
...
#elif _OSX
		a4.s_addr = ((const uint32_t *)addr)[3];
#else
		a4.s_addr = addr->__in6_u.__u6_addr32[3];
#endif
		len = snprintf(tmp, sizeof(tmp), "::%s%s", (i != 0) ? "ffff:" : "", mmt_inet_ntop4(&a4, tmp2, sizeof(tmp2)));
		if (len >= size)
			return NULL;
...
const char *mmt_inet_ntop(int af, const void *addr, char *buf, socklen_t len)
{
	if (af == AF_INET6)
