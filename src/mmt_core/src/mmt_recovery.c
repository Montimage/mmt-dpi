 ...
 */
static uint32_t calculate_retry_delay(
    uint32_t attempt,
    const mmt_retry_config_t *config)
{
    ...
    mmt_fallback_strategy_t strategy)
{
    MMT_LOG_DEBUG_CAT(MMT_LOG_CAT_PROTOCOL,
                      "Attempting protocol fallback: proto=%u, strategy=%d",
                      protocol_id, strategy);
    ...
    switch (strategy) {
        ...
        case MMT_FALLBACK_GENERIC:
            MMT_LOG_INFO_CAT(MMT_LOG_CAT_PROTOCOL,
                            "Using generic parser for protocol %u", protocol_id);
        ...
        case MMT_FALLBACK_NEXT_LAYER:
            MMT_LOG_INFO_CAT(MMT_LOG_CAT_PROTOCOL,
                            "Skipping to next protocol layer");
            g_recovery_stats.successful_recoveries++;
        ...
        case MMT_FALLBACK_ALTERNATIVE:
            ...
            if (protocol_id < MAX_FALLBACK_HANDLERS &&
                g_fallback_handlers[protocol_id] != NULL) {
                MMT_LOG_INFO_CAT(MMT_LOG_CAT_PROTOCOL,
                                "Using custom fallback handler");
                mmt_recovery_result_t result =
                    g_fallback_handlers[protocol_id](packet, offset);
        ...
        case MMT_FALLBACK_RAW:
            MMT_LOG_INFO_CAT(MMT_LOG_CAT_PROTOCOL,
                            "Treating as raw data");
            g_recovery_stats.successful_recoveries++;
            return MMT_RECOVERY_SUCCESS;

        default:
            MMT_LOG_ERROR_CAT(MMT_LOG_CAT_PROTOCOL,
                             "Unknown fallback strategy: %d", strategy);
            g_recovery_stats.failed_recoveries++;
            return MMT_RECOVERY_FAILED;
    }
}
...
{
    if (protocol_id >= MAX_FALLBACK_HANDLERS) {
        MMT_RETURN_ERROR(MMT_ERROR_INVALID_PROTOCOL,
                        "Protocol ID out of range");
    }

    if (handler == NULL) {
        MMT_RETURN_ERROR(MMT_ERROR_INVALID_PARAMETER,
                        "Handler is NULL");
    }
    ...
    MMT_LOG_DEBUG_CAT(MMT_LOG_CAT_PROTOCOL,
                      "Registered fallback handler for protocol %u",
                      protocol_id);
...
{
    MMT_LOG_DEBUG_CAT(MMT_LOG_CAT_SESSION,
                      "Attempting session recovery: error=%d, strategy=%d",
                      error, strategy);
    ...
    switch (strategy) {
        ...
        case MMT_SESSION_RECOVERY_SKIP:
            MMT_LOG_INFO_CAT(MMT_LOG_CAT_SESSION,
                            "Skipping session operation");
            g_recovery_stats.successful_recoveries++;
            return MMT_RECOVERY_SKIP;

        default:
            MMT_LOG_ERROR_CAT(MMT_LOG_CAT_SESSION,
                             "Unknown recovery strategy: %d", strategy);
            g_recovery_stats.failed_recoveries++;
            return MMT_RECOVERY_FAILED;
    }
}
    ...
    const mmt_retry_config_t *retry_config)
{
    ...
    MMT_LOG_DEBUG("Executing operation with retry: max_retries=%u",
    ...
    for (attempt = 1; attempt <= retry_config->max_retries; attempt++) {
        ...
        MMT_LOG_DEBUG("Retry attempt %u/%u after %ums delay",
                      attempt, retry_config->max_retries, delay_ms);
        ...
        MMT_LOG_WARN("Retry attempt %u failed: %s",
                     attempt, mmt_error_to_string(result));
    }
...
mmt_error_t mmt_session_mark_degraded(const void *session_key)
{
    ...
    MMT_LOG_WARN_CAT(MMT_LOG_CAT_SESSION,
                     "Session marked as degraded: hash=%u", hash);
...
mmt_error_t mmt_session_restore(const void *session_key)
{
    ...
    if (!g_degraded_sessions[hash]) {
        MMT_LOG_DEBUG_CAT(MMT_LOG_CAT_SESSION,
                         "Session not degraded, no restore needed");
        return MMT_SUCCESS;
    }
    ...
    MMT_LOG_INFO_CAT(MMT_LOG_CAT_SESSION,
                     "Session restored from degraded state: hash=%u", hash);
...
void mmt_recovery_print_stats(void)
{
    ...
    if (g_recovery_stats.protocol_fallbacks + g_recovery_stats.session_recoveries > 0) {
        uint64_t total = g_recovery_stats.protocol_fallbacks +
                        g_recovery_stats.session_recoveries;
        double success_rate = (double)g_recovery_stats.successful_recoveries /
                             total * 100.0;
